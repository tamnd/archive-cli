...
func newCacheCmd(app *App) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "archive caches item metadata, search pages, views, and Wayback availability so repeated commands stay fast. Manage that cache here.",
	}
	cmd.AddCommand(
		&cobra.Command{
			Use:   "dir",
		...
		&cobra.Command{
			...
			RunE: func(c *cobra.Command, _ []string) error {
				if !confirm(app.yes, "clear the archive cache?") {
