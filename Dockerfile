...
# ca-certificates for HTTPS to archive.org; tzdata for sane timestamps.
RUN apk add --no-cache ca-certificates tzdata \
 && adduser -D -H -u 10001 archive \
 && mkdir -p /data \
 && chown archive:archive /data

COPY $TARGETPLATFORM/archive /usr/bin/archive

USER archive
WORKDIR /data

# All state lives under /data; mount a volume to keep downloads and the cache:
#
#   docker run -v ~/data/archive:/data ghcr.io/tamnd/archive search nasa
ENV ARCHIVE_DATA_DIR=/data
VOLUME ["/data"]

ENTRYPOINT ["/usr/bin/archive"]
